...
| Subagents | **Yes (Claude Code, OpenCode, Codex, Antigravity)** | `pr-fixer` and `routing-judge` give context isolation per iteration. Each supported orchestrator gets its native format: Claude Code Markdown with `name:`, OpenCode Markdown with `mode: subagent`, Codex TOML custom agents, and Antigravity Markdown agent personas. Worker CLIs can still be invoked as plain CLIs through `invoke-worker.sh`. |
| Distribution | **`npx skills` + post-install bootstrap** | `npx skills add araozmd/multi-cli-orchestrator` installs the skill folders, but the skills CLI does not relocate bundled subagents into each CLI's native agents dir (`~/.claude/agents/`, `~/.config/opencode/agents/`, `~/.codex/agents/`, `~/.gemini/config/agents/`). A one-shot bootstrap script (shipped at `skills/start-feature/scripts/install-agents.sh` so it rides along with the skills install) symlinks them, namespaced as `<skill>-<agent>.<ext>`. Idempotent; re-runnable after updates. |
...
| `routing-judge` subagent | `skills/route-task/agents/{claude-code,opencode,antigravity}/routing-judge.md`, `skills/route-task/agents/codex/routing-judge.toml` | Pure decision agent for routing; returns `{worker, rationale, prompt}`. CLI-specific native variants. |
| `pr-loop` skill | `skills/pr-loop/SKILL.md` | Drive the Codex review cycle and auto-merge. |
| `pr-fixer` subagent | `skills/pr-loop/agents/{claude-code,opencode,antigravity}/pr-fixer.md`, `skills/pr-loop/agents/codex/pr-fixer.toml` | Fix one Codex comment per invocation in isolated context. CLI-specific native variants. |
| `invoke-worker.sh` | `skills/start-feature/scripts/invoke-worker.sh` | Single chokepoint for worker CLI invocation. Future A2A swap point. Lives inside `start-feature` so `npx skills` installs it (sibling top-level dirs aren't copied). Runtime path: `~/.agents/skills/start-feature/scripts/invoke-worker.sh`. |
| `install-agents.sh` | `skills/start-feature/scripts/install-agents.sh` | Post-install bootstrap. Symlinks bundled subagents into each CLI's native agents dir under a `<skill>-<agent>.<ext>` namespace. Same in-skill location for the same distribution reason. |
...
- Targets all 50+ agents the `skills` CLI supports for the skill files themselves. Subagents are bundled inside skill folders, but the skills CLI does not auto-register them with each CLI's native subagent directory; users run `bash ~/.agents/skills/start-feature/scripts/install-agents.sh` once after install (and after any update that ships new subagents) to symlink them into Claude Code, OpenCode, Codex, and Antigravity native agent directories. The bootstrap is namespaced (`<skill>-<agent>.<ext>`) so multiple skills can share an agent name without colliding. The bootstrap and the worker harness ship inside `skills/start-feature/scripts/` because the `npx skills` CLI only copies `skills/<name>/` subtrees — top-level sibling files (e.g. a repo-root `scripts/` dir) would never reach the consumer.
