...
- `skills/start-feature/scripts/invoke-worker.sh` — **single chokepoint** for every worker CLI call. Shells out to `claude`/`codex`/`opencode`/`gemini` with the right headless-approval flags. Codex uses the native `codex exec` CLI (Plus subscription). Future Phase 4: drop-in A2A client wrapper. **Keep its signature stable** (`<worker> <prompt-file> [log-dir]`) — that stability is what makes the A2A swap a one-file change. Lives inside `start-feature` (rather than at the repo root) so `npx skills` installs it; runtime path is `~/.agents/skills/start-feature/scripts/invoke-worker.sh`.
- `skills/start-feature/scripts/install-agents.sh` — post-install bootstrap. The `npx skills` CLI installs skill folders verbatim but does **not** register bundled subagents with Claude Code (`~/.claude/agents/`) or OpenCode (`~/.config/opencode/agents/`). This script walks `<skills-dir>/*/agents/<cli>/*.md` and symlinks each one as `<skill>-<agent>.md` into the right dir. Idempotent. Re-run after `npx skills update` adds new subagents. Use `--unlink` to remove, `--dry-run` to preview. Runtime path: `~/.agents/skills/start-feature/scripts/install-agents.sh`.
...
Skills are consumed by `vercel-labs/skills` CLI, which expects the layout `skills/<name>/SKILL.md` with frontmatter (`name`, `description`). Subagents live under `skills/<name>/agents/<cli>/<agent>.md` (per-CLI subdir) so they ride along with the skill folder. Anything else the orchestrator needs at runtime (the worker harness, the bootstrap) also lives inside a skill folder — `skills/start-feature/scripts/` — because the `npx skills` CLI **only** installs `skills/<name>/` subtrees, not sibling top-level dirs. Targeting flags: `-a claude-code -a opencode -g`. Subagent registration is **not** automatic — `bash ~/.agents/skills/start-feature/scripts/install-agents.sh` is the second step users must run.
...
- `bash ~/.agents/skills/start-feature/scripts/install-agents.sh` (idempotent) — symlink bundled subagents into `~/.claude/agents/` and `~/.config/opencode/agents/`. Use `--dry-run` to preview, `--unlink` to remove.
